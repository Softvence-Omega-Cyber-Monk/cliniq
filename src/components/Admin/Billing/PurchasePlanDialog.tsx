...
import { usePurchaseSubscriptionMutation, SubscriptionPlan } from "@/store/api/billingApi";
...
const PurchasePlanDialog: React.FC<PurchasePlanDialogProps> = ({
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            alert(`Successfully started ${selectedPlan?.planName} plan!`);
            onSuccess?.();
            onOpenChange(false);
        } catch (err: any) {
            alert(`Failed to purchase plan: ${err?.data?.message || err.message}`);
        }
    };
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-4xl bg-white">
                    ...
                    {plans
                        ...
                        .map((plan) => {
                            ...
                            return (
                                <Card
                                    key={plan.id}
                                    className={`cursor-pointer transition-all hover:shadow-xl ${isSelected ? "ring-2 ring-teal-500 border-teal-500" : ""
                                        }`}
                ...
                <DialogFooter className="flex justify-center">
                    <Button
                        size="lg"
                        className="px-12 bg-teal-600 hover:bg-teal-700 text-lg"
