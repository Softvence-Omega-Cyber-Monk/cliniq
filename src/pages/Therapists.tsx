...
import { Search, ChevronDown, Plus, User, Mail, Phone, BookOpen, Clock, Briefcase, ChevronLeft, Calendar, FileText, X } from 'lucide-react';
...
const MOCK_THERAPISTS: Therapist[] = [
  { id: 1, name: 'Dr. Michael Chen', initials: 'MC', specialty: 'Cognitive Behavioral Therapy', status: 'Inactive', patients: 12, isCurrentUser: true },
  { id: 2, name: 'Dr. Sarah Johnson', initials: 'SJ', specialty: 'Mindfulness Therapy', status: 'Inactive', patients: 8 },
  { id: 3, name: 'Dr. Emily Rodriguez', initials: 'ER', specialty: 'Psychodynamic Therapy', status: 'Inactive', patients: 15 },
  { id: 4, name: 'Dr. Liam Davis', initials: 'LD', specialty: 'Cognitive Behavioral Therapy', status: 'Inactive', patients: 12 },
  { id: 5, name: 'Dr. John Doe', initials: 'JD', specialty: 'Family Therapy', status: 'Active', patients: 25 },
  { id: 6, name: 'Dr. Olivia Wilson', initials: 'OW', specialty: 'Cognitive Behavioral Therapy', status: 'Inactive', patients: 12 },
  { id: 7, name: 'Dr. Alice Brown', initials: 'AB', specialty: 'Exposure Therapy', status: 'Active', patients: 18 },
];

const MOCK_PATIENTS: Patient[] = [
    { id: 1, name: 'Alex Johnson', sessionCount: 4, progressPercent: 50, status: 'Active', therapistId: 1 },
    { id: 2, name: 'Emily Carter', sessionCount: 15, progressPercent: 50, status: 'Active', therapistId: 1 },
    { id: 3, name: 'Michael Smith', sessionCount: 7, progressPercent: 50, status: 'Active', therapistId: 1 },
    { id: 4, name: 'Sophia Brown', sessionCount: 3, progressPercent: 50, status: 'Inactive', therapistId: 1 },
    { id: 5, name: 'Liam Davis', sessionCount: 6, progressPercent: 50, status: 'Active', therapistId: 2 },
    { id: 6, name: 'Olivia Wilson', sessionCount: 3, progressPercent: 50, status: 'Active', therapistId: 2 },
    { id: 7, name: 'James Taylor', sessionCount: 8, progressPercent: 50, status: 'Inactive', therapistId: 3 },
    { id: 8, name: 'Isabella Martinez', sessionCount: 6, progressPercent: 50, status: 'Active', therapistId: 3 },
    { id: 9, name: 'Ethan Anderson', sessionCount: 11, progressPercent: 50, status: 'Inactive', therapistId: 4 },
    { id: 10, name: 'Ava Thomas', sessionCount: 9, progressPercent: 50, status: 'Active', therapistId: 5 },
];
...
const AddTherapistModal: React.FC<AddTherapistModalProps> = ({
  ...
  const handleSave = async () => {
    if (!profile.name || !profile.speciality || !profile.email) {
      setSaveMessage({ type: 'error', message: 'Please fill in required fields (Name, Speciality, Email).' });
    ...
    setIsSaving(false);
    setSaveMessage({ type: 'success', message: isEditMode ? 'Profile updated successfully!' : 'Therapist added successfully!' });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-transform duration-300 scale-100">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 rounded-t-2xl p-6 pb-4 z-10">
          <div className="flex justify-between items-center mb-2">
            <h1 className="text-2xl font-bold text-gray-900">{isEditMode ? 'Edit Personal Info' : 'Add New Therapist'}</h1>
            <button
              onClick={onClose}
          ...
          <p className="text-gray-500 text-sm">{isEditMode ? 'Update your professional information and availability' : 'Enter the professional information and availability for the new therapist.'}</p>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">{profile.name || 'Therapist Name'}</h3>
              <p className="text-sm text-gray-500">{profile.speciality || 'Speciality'}</p>
            </div>
          </div>
          ...
          <FormInput label="Qualifications & Certifications" name="qualifications" value={profile.qualifications} onChange={handleChange} isTextarea />

          <div className="space-y-4">
            <div>
              <p className="text-sm font-medium text-gray-700 mb-3">Available Days</p>
              <div className="flex flex-wrap gap-2">
                {ALL_DAYS.map((day) => (<DayChip key={day} day={day} />))}
              </div>
            </div>
...
const DetailPatientRow: React.FC<DetailPatientRowProps> = ({ patient, onViewPatientDetails }) => {
    const statusClasses = patient.status === 'Active'
...
const TherapistDetail: React.FC<TherapistDetailProps> = ({ therapist, onBack, setIsEditModalOpen, onViewPatientDetails }) => {
    
    // Filter patients specific to this therapist (mock)
    const therapistPatients = useMemo(() => MOCK_PATIENTS.filter(p => p.therapistId === therapist.id), [therapist.id]);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center space-x-2 text-sm text-gray-500 mb-6">
                <button onClick={onBack} className="text-teal-600 hover:text-teal-800 flex items-center space-x-1 font-medium">
                    <ChevronLeft size={16} />
                    <span>Therapists</span>
                </button>
                <span>/</span>
                <span className="font-semibold text-gray-800">{therapist.name} ({therapist.initials})</span>
            </div>
                ...
                <div className="flex justify-between items-start border-b pb-5 mb-5">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            {mockDetail.isCurrentUser && <span className="text-sm font-normal text-gray-500">(Me)</span>}
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-5 rounded-xl shadow-lg flex items-center justify-between border-l-4 border-gray-500">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" value="" className="sr-only peer" defaultChecked={therapist.status === 'Active'} />
...
const SessionNoteModal: React.FC<SessionNoteModalProps> = ({ isOpen, onClose, session }) => {
    ...
    return (
        <div 
            ...
            role="dialog"
        >
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto transform transition-transform duration-300 scale-100"
            ...
            >
                ...
                <div className="sticky top-0 bg-white border-b border-gray-200 rounded-t-2xl p-6 pb-4 z-10">
                    ...
                    <p className="text-gray-500 text-sm">Detailed information about this therapy session</p>
                </div>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="flex justify-between items-center bg-gray-50 p-4 rounded-xl border border-gray-100">
                        ...
                        <span className="text-sm font-semibold px-2 py-0.5 rounded bg-teal-100 text-teal-700">{session.duration} min</span>
                    </div>

                    {/* Session Notes */}
                    <div className="space-y-3">
                        <h3 className="text-lg font-bold text-gray-900 border-b pb-2">Session Notes</h3>
                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-inner max-h-80 overflow-y-auto">
                            <p className="text-gray-700 whitespace-pre-wrap leading-relaxed">{session.fullNote}</p>
                        </div>
                    </div>
                    
                    {/* Quick Summary */}
                    <div className="space-y-2">
                        <h3 className="text-lg font-bold text-gray-900 border-b pb-2">Quick Summary</h3>
...
const SessionHistoryCard: React.FC<SessionHistoryProps> = ({ date, summary, duration, fullNote, onViewNote }) => {
    ...
    return (
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition">
            ...
            <div className="flex justify-between items-start space-x-4 mb-3">
                ...
                <span className="text-xs font-semibold px-2 py-0.5 rounded bg-gray-100 text-gray-600 flex-shrink-0">{duration} min</span>
            </div>
...
const PatientDetail: React.FC<PatientDetailProps> = ({ patient, therapist, onBackToTherapist }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-teal-500">
                <h2 className="text-xl font-bold text-gray-900 mb-4 border-b pb-3">Patient Information</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                    ...
                    <div className="space-y-3">
                        ...
                        <DetailItem icon={Phone} label="Phone Number" value={mockPatientData.phone} />
                        <DetailItem icon={Phone} label="Emergency Contact" value={`${mockPatientData.emergencyContactName}, ${mockPatientData.emergencyContactPhone}`} />
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-2xl font-extrabold text-teal-600">{mockPatientData.overallProgress}%</span>
                </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg">
                ...
                <div className="space-y-5">
                    {mockPatientData.treatmentGoals.map((goal, index) => (
                        <TreatmentGoal key={index} title={goal.title} progress={goal.progress} />
                    ))}
                </div>
            </div>
...
const TherapistCard: React.FC<{ therapist: Therapist, onViewDetails: (id: number) => void }> = ({ therapist, onViewDetails }) => {
  const statusClasses = therapist.status === 'Active'
    ? 'bg-green-100 text-green-700'
    : 'bg-red-100 text-red-700';
  
  const initialColor = (therapist.initials === 'MC' || therapist.initials === 'SJ') ? 'bg-blue-500' : 'bg-green-500';
  ...
  return (
    <div className="bg-white p-5 rounded-xl shadow-md border border-gray-100 flex flex-col transition-shadow hover:shadow-lg">
      ...
      <div className="flex items-center space-x-3 mb-4">
        <div className={`w-8 h-8 flex items-center justify-center text-sm font-bold text-white rounded-full ${initialColor}`}>
          {therapist.initials}
        </div>
        <div>
          <h3 className="text-lg font-semibold text-gray-800">
            {therapist.name} {therapist.isCurrentUser && <span className="text-sm font-normal text-gray-500">(ME)</span>}
          </h3>
      ...
      <div className="flex justify-between items-center mb-5 border-t pt-4">
        ...
        <div className="flex flex-col items-end">
          ...
          <span className="text-2xl font-bold text-gray-800">{therapist.patients}</span>
        </div>
      </div>
...
const App: React.FC = () => {
  ...
  const [selectedPatientId, setSelectedPatientId] = useState<number | null>(null); // NEW STATE for patient detail

  const handleAddTherapist = (profile: UserProfile) => {
    const newTherapist: Therapist = {
      id: therapists.length + 1,
      name: profile.name,
      initials: profile.name.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2),
  ...
  const isPatientDetailView = selectedPatientId !== null && selectedPatient && selectedTherapist;
      ...
      {isListView && (
        <>
          <header className="flex justify-between items-start mb-8">
            <div>
              <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">THERAPISTS</h1>
              <p className="text-gray-500 text-sm mt-1">Manage therapists in your practice.</p>
            </div>
            <button 
