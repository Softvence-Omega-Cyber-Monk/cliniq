...
const plans: Plan[] = [
  {
    ...
    price: 29,
    period: '/month',
    features: ['Up to 15 clients', 'Session management', 'Basic reporting', 'Email support'],
    isCurrent: false,
  ...
  },
  {
    ...
    price: 49,
    period: '/month',
    features: ['Up to 50 clients', 'Advanced session tools', 'Full analytics & reports', 'Crisis alert system', 'Priority support'],
    isCurrent: true,
  ...
  },
  {
    ...
    price: 79,
    period: '/month',
    features: ['Unlimited clients', 'All Professional features', 'AI-powered insights', 'Custom integrations', 'Dedicated support'],
    isCurrent: false,
    buttonText: 'Upgrade',
    isPopular: false,
  },
...
const notifications: Notification[] = [
  { title: 'Payment reminders', desc: 'Get notified if 7 days before payment', enabled: true },
  { title: 'Payment confirmations', desc: 'Get notified upon successful payment', enabled: true },
  { title: 'Plan changes', desc: 'Notifications for subscription updates', enabled: false },
];

// --- ICONS (Simulated Lucide Icons for single-file self-containment) ---

const CheckIcon: React.FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-teal-500 mr-2 flex-shrink-0">
...
const InfoIcon: React.FC<{ size?: number, className?: string }> = ({ size = 18, className = "" }) => (
...
const PlanCard: React.FC<{ plan: Plan }> = ({ plan }) => {
  ...
  return (
    <div className={`p-6 rounded-2xl flex flex-col h-full bg-white ${cardClasses}`}>
      {plan.isPopular && (
      ...
      <button
        ...
        disabled={isCurrent}
        onClick={() => !isCurrent && console.log(`${plan.buttonText} ${plan.name}`)}
      >
        {plan.buttonText}
      </button>
    </div>
...
const App: React.FC = () => {
  ...
  const [currentNotifications, setNotifications] = useState<Notification[]>(notifications);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8 font-inter">
      <style>{`
        /* Custom font import for pixel perfect design */
        ...
        /* Custom toggle track color to match design */
        input[type="checkbox"]:checked + div {
            background-color: #00C6AE; /* Custom Teal */
        }
        
        .visa-bg {
            background-color: #1a4f91; /* Dark Blue for Visa look */
        }
      `}</style>
      ...
      <main className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="mb-8">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-wide">BILLING & SUBSCRIPTION</h1>
          <p className="text-sm text-gray-500 mt-1">Manage your practice's subscription plan and billing information</p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 pt-4">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
            ...
            <div className="space-y-4">
              {paymentMethods.map((method) => (
                <div key={method.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-xl hover:shadow-sm transition-shadow">
                  <div className="flex items-center">
                    ...
                    <div className="text-sm">
                      <p className="font-medium text-gray-800">XXXX XXXX {method.lastFour}</p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Billing Notifications</h2>
        ...
        <div className="pt-4">
          <h2 className="text-lg font-semibold text-gray-800 mb-6">Compare Plans</h2>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead>
                <tr className="text-left text-sm font-medium text-gray-500">
                  <th className="py-3 pr-4 whitespace-nowrap w-2/5 md:w-1/4">Invoice</th>
              ...
              <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
                {currentInvoices.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                    <td className="py-3 pr-4 font-medium text-gray-900 whitespace-nowrap">Invoice #{invoice.id}</td>
                    <td className="py-3 pr-4 whitespace-nowrap">{invoice.date}</td>
                    <td className="py-3 pr-4 whitespace-nowrap">${invoice.amount.toFixed(0)}</td>
