...
const ToggleItem: React.FC<ToggleItemProps> = ({ label, description, isEnabled, onToggle }) => (
...
const ProfileSection: React.FC = () => {
  ...
  const [avatarUrl, setAvatarUrl] = useState<string>('https://placehold.co/80x80/2f4f4f/ffffff?text=AJ');
  const fileInputRef = useRef<HTMLInputElement>(null);

  const handleChange = useCallback((key: keyof typeof profileData) => (e: React.ChangeEvent<HTMLInputElement>) => {
...
const App: React.FC = () => {
  ...
  const handleSecurityToggle = useCallback((key: keyof typeof securitySettings) => {
    setSecuritySettings(prev => ({ ...prev, [key]: !prev[key] }));
  }, []);

  const handleNotificationToggle = useCallback((key: keyof typeof notificationSettings) => {
